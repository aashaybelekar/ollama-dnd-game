...
def start_adventure():
    ...
    if not has_adventure_started:
        st.session_state["characters_in_adventure"] = st.multiselect(
            "Which hero will you choose for this adventure?", characters.keys())

        adventure_difficulty = st.radio(
            "Select Difficulty", ["Easy", "Medium", "Hard"], key="adventure_difficulty")

        if st.button("Set Adventure Prompt"):
            adventure_user_prompt = st.text_area(
                "Adventure Prompt", key="appearance")
        ...
        if st.session_state["characters_in_adventure"]:
            if st.button("Start Adventure"):
                with st.spinner("Creating World.."):
                    adventure_start = st.write_stream(st.session_state["api"].start_adventure(
                        st.session_state["characters_in_adventure"], adventure_difficulty, adventure_user_prompt))

                st.session_state["has_adventure_started"] = True

                with st.spinner("Thinking of a name for this adventure.."):
                    adventure_name = st.session_state['api'].name_adventure(
                        adventure_start)
                    adventure_name = adventure_name.replace("\"", "")

                st.session_state["current_uuid"] = str(uuid4())

                st.session_state['adventure_dict'][st.session_state["current_uuid"]
                                                   ] = adventure_name
                st.session_state["chat_history"].append(
                    (None, SystemMessage(adventure_start)))

                history_document = Document(page_content=adventure_start, metadata={
                                            'source': "AI", "uuid": st.session_state["current_uuid"]})

                st.session_state['api'].save_doc_to_history_vector(
                    history_document)

                with st.spinner("Saving.."):
                    save_adventure(st.session_state["current_uuid"], history=st.session_state["chat_history"],
                                   history_names=st.session_state['adventure_dict'], characters=st.session_state["characters_in_adventure"])
                st.rerun()
    ...
    if has_adventure_started:
        ...
        if submit_button and chat_message:
            ...
            for character, message in chat_message.items():  # Done before processing to display the text on the UI
                if message:
                    st.session_state["chat_history"].append(
                        (character, HumanMessage(message)))

            with st.spinner("Thinking.."):
                adventure_progression = st.write_stream(st.session_state["api"].progress_story(
                    chat_message, st.session_state.temp_chat_state))

            for character, message in chat_message.items():
                if message:
                    history_document = Document(page_content=message, metadata={
                                                'source': f"{character}", "uuid": st.session_state["current_uuid"]})
                    with st.spinner("Saving.."):
                        save_adventure(
                            st.session_state["current_uuid"], history=st.session_state["chat_history"], history_names=st.session_state['adventure_dict'])
                        st.session_state['api'].save_doc_to_history_vector(
                            history_document)

            st.session_state['chat_history'].append(
                (None, AIMessage(adventure_progression)))
            history_document = Document(page_content=adventure_progression, metadata={
                                        'source': "AI", "uuid": st.session_state["current_uuid"]})

            with st.spinner("Saving.."):
                save_adventure(st.session_state["current_uuid"], history=st.session_state["chat_history"],
                               history_names=st.session_state['adventure_dict'])
                st.session_state['api'].save_doc_to_history_vector(
                    history_document)
