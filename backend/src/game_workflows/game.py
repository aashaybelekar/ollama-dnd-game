...
def play_game():
    if 'dm_model' not in st.session_state or 'vector_store' not in st.session_state or 'history_store' not in st.session_state:
        st.warning(
            "Please set up your models in the 'Manage Models' section before playing.")
        return
...
def initialize_adventure_state(uuid):
    st.session_state["chat_history"], st.session_state["characters_in_adventure"] = load_adventure(
        uuid=uuid)
